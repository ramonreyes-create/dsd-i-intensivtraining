...
<body class="__BODY_CLASS__">
  ...
  <script>
    ...
    function downloadReport(){const rows=filteredReportRows();if(!rows.length)return toast('Für diese Auswahl gibt es keine Daten.');const csv=['Datum;Name;Kurs;Aktivität;Ergebnis;Text',...rows.map(r=>[r.created_at,r.name,r.course,r.activity,r.result||'',reportDetail(r)].map(v=>'"'+String(v).replaceAll('"','""')+'"').join(';'))].join('\n'),a=document.createElement('a');a.href=URL.createObjectURL(new Blob(['\ufeff'+csv],{type:'text/csv'}));a.download='DSD-I-Schueleraktivitaeten.csv';a.click();URL.revokeObjectURL(a.href)}
    loadMaterials();
  </script>
</body>
</html>`;
